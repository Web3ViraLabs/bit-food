...
var ServiceAreaProvider = {
  ...
  'get': function (callback) {
    ...
    FetchUtil.multiFetch(function (responseMap, errorMap) {
      ...
      if (ipInfo) {
        ipInfo = ipInfo.body ? ipInfo.body.data : null;
      }

      if (serviceAreaList) {
        serviceAreaList = serviceAreaList.body ? serviceAreaList.body.rows : [];
        for(var saIndex = 0; saIndex < serviceAreaList.length; saIndex++){
            if(serviceAreaList[saIndex].serviceProviderAvailableCountryModel.countryModel.name){
                if(COUNTRIES_TO_HIDE.indexOf(serviceAreaList[saIndex].serviceProviderAvailableCountryModel.countryModel.name) > -1){
                    serviceAreaList.splice(saIndex, 1);
                    saIndex--;
                }
            }
        }
